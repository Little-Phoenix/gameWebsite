...
export const TAB_CHEDCKED = 'TAB_CHEDCKED'
export const FETCH_ERROR = 'FETCH_ERROR'
export const CUT_DOWN = 'CUT_DOWN'
...
export function tabChange(newTab, oldTab, cb) {
	return {
		type: TAB_TOGGLE,
		oldTab,
		newTab,
...
export function tabChecked(tabNum) {
	return {
		type: TAB_CHEDCKED,
		tabNum
	}
...
export function cutDown(cutDownInfo) {
	// 倒计时
	return {
		type: CUT_DOWN,
		cutDownInfo
...
export function tick(resetDelay) {
	// 倒计时--
	return {
		type: TICK,
		resetDelay
...
export function getUnread(val) {
	return {
		type: GET_UNREAD,
		val: val || localStorage.getItem('unread_num') || 0
	}
}
